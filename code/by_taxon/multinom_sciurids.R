...
for(i in 1:nrow(tr)) {
  sciurids.i <- sciu.df %>% filter(Time==tr$Time[i] & Region==tr$Region[i])
  for(s in 1:n.spp) {
    sp.i <- sciurids.i %>% filter(Species==unique(Species)[s])
...
all.df <- do.call("rbind", NZ.ls) %>%
  mutate(spp=factor(spp, levels=1:n.spp, labels=unique(sciu.df$Species)))
