...
for(i in 1:nrow(tr)) {
  mice.i <- mice.df %>% filter(Time==tr$Time[i] & Region==tr$Region[i])
  for(s in 1:n.spp) {
    sp.i <- mice.i %>% filter(Species==unique(Species)[s])
...
all.df <- do.call("rbind", NZ.ls) %>%
  mutate(spp=factor(spp, levels=1:n.spp, labels=unique(mice.df$Species)))
