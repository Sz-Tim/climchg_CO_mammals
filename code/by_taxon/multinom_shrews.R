...
for(i in 1:nrow(tr)) {
  shrew.i <- shrew.df %>% filter(Time==tr$Time[i] & Region==tr$Region[i])
  for(s in 1:n.spp) {
    sp.i <- shrew.i %>% filter(Species==unique(Species)[s])
...
all.df <- do.call("rbind", NZ.ls) %>%
  mutate(spp=factor(spp, levels=1:n.spp, labels=unique(shrew.df$Species)))
